<x-app-layout>
    <!-- 
        NOTA: Asegúrate de tener Alpine.js cargado en tu <head> o antes de cerrar el <body>
        ...
        <div x-data="{ view: 'table' }" class="max-w-7xl w-full px-4 sm:px-6 lg:px-8">
            <div class="text-gray-900 dark:text-gray-100">
                                ...
                                <div class="p-4">
                                    <form action="{{ route('resumen') }}" method="GET" class="space-y-4">
                                        ...
                                        <div class="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-2">
                                            ...
                                            <a href="{{ route('resumen') }}" class="block w-full text-center text-sm font-medium text-indigo-600 dark:text-indigo-400 hover:underline">Limpiar Filtros</a>
                                            -->
                                        </div>
                                    </form>
                                </div>
                ...
                @else
                    ...
                    <!-- VISTA DE TABLA CON ESTILO SLATE -->
                    <div x-show="view === 'table'" x-transition
                         class="bg-slate-900/70 rounded-2xl p-4">
                        <div class="overflow-x-auto rounded-lg">
                            <table class="min-w-full text-sm">
                                <thead class="bg-slate-700">
                                    <tr>
                                        <th scope="col" class="px-4 py-2 text-left font-semibold text-gray-300">Cliente</th>
                                        <th scope="col" class="px-4 py-2 text-left font-semibold text-gray-300">Destino</th>
                                        <th scope="col" class="px-4 py-2 text-left font-semibold text-gray-300">Fecha Salida</th>
                                        <th scope="col" class="px-4 py-2 text-center font-semibold text-gray-300">Micros</th>
                                        <th scope="col" class="px-4 py-2 text-right font-semibold text-gray-300">Monto</th>
                                        <th scope="col" class="px-4 py-2 text-center font-semibold text-gray-300">Estado</th>
                                        <th scope="col" class="px-4 py-2 text-right font-semibold text-gray-300">Acciones</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-700">
                                    @foreach ($reservas as $reserva)
                                        <tr class="bg-slate-800 hover:bg-slate-700/50 transition-colors">
                                            <td class="px-4 py-2 whitespace-nowrap font-medium text-gray-300">{{ $reserva->cliente->nombre ?? '' }} {{ $reserva->cliente->apellido ?? '' }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">{{ $reserva->lugar->nombre ?? 'N/A' }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-gray-300">{{ optional($reserva->horario)->fecha_salida ? \Carbon\Carbon::parse($reserva->horario->fecha_salida)->format('d/m/Y H:i') : 'N/A' }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-center text-gray-300">{{ $reserva->micros->sum('cantidad') }}</td>
                                            <td class="px-4 py-2 whitespace-nowrap text-right text-gray-300">${{ number_format($reserva->monto, 0, ',', '.') }}</td>
                                            ...
                                            <td class="px-4 py-2 whitespace-nowrap text-right">
                                                ...
                                                <div class="flex justify-end gap-3">
                                                    <a href="{{ route('clientes.show', $reserva->cliente->id) }}" class="text-gray-400 hover:text-white" title="Ver">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10 12a2 2 0 100-4 2 2 0 000 4z" /><path fill-rule="evenodd" d="M.458 10C1.732 5.943 5.522 3 10 3s8.268 2.943 9.542 7c-1.274 4.057-5.064 7-9.542 7S1.732 14.057.458 10zM14 10a4 4 0 11-8 0 4 4 0 018 0z" clip-rule="evenodd" /></svg>
                                                    </a>
                                                    <a href="{{ route('reservas.edit', $reserva->id) }}" class="text-indigo-400 hover:text-indigo-300" title="Editar Reserva">
                                ...
                                <div class="space-y-1 text-sm text-gray-700 dark:text-gray-200">
                                    ...
                                    @if ($reserva->micros->isNotEmpty())
                                        <!-- // Se comenta debido a que pueden haber muchos tipos de micros lo que afecta el que se entienda de primera mano
                                            <p><span class="font-semibold">Tipo Micro:</span></p>
                                        -->
