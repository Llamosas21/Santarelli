...
class ReservaApiController extends Controller
{
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($data, [
            ...
            // <-- 3. AÑADIR VALIDACIÓN DE PARTIDA -->
            'partida' => 'required|string|min:5|max:255',
        ...
        // --- 3. LÓGICA DE NEGOCIO Y MODELOS ---

        try {
            // <-- 4. AÑADIR $partidas_validas AL 'use' -->
            $reservaGuardada = DB::transaction(function () use ($request, $micros_validos, $destinos_validos) {

                // --- A. CREAR CLIENTE Y CONTACTO ---
                ...
                // <--  OBTENER LUGARPARTIDA -->
                $partida_nombre = $request->input('partida');
                $lugarPartida = LugarPartida::firstOrCreate(['nombre' => $partida_nombre]);
              
                // --- C. PROCESAR MICROS Y CALCULAR TOTALES ---
                $micros_input = $request->input('micro');
                $cantidades_input = $request->input('cantidad_micros');
                ...
                $reserva = Reserva::create([
                    ...
                    // <-- 6. AÑADIR ID DE PARTIDA A LA RESERVA -->
                    'lugar_partida_id' => $lugarPartida->id,
                    'cantidad_pasajeros' => $pasajeros_total_final,
